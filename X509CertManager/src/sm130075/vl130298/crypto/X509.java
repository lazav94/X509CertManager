 ...
 * @see X509CertInfo
 */
public class X509 extends X509Certificate implements DerEncoder {
     ...
     */
    public X509(byte[] certData) throws CertificateException {
        try {
     ...
     * @exception CertificateException on parsing and initialization errors.
     */
    public X509(InputStream in) throws CertificateException {
     ...
     *              created from.
     */
    public X509(X509CertInfo certInfo) {
        this.info = certInfo;
    }
     ...
     * @exception CertificateException on parsing and initialization errors.
     */
    public X509(DerValue derVal) throws CertificateException {
        try {
            parse(derVal);
        } catch (IOException e) {
            signedCert = null;
    ...
    public Set<String> getCriticalExtensionOIDs() {
        ...
        try {
            ...
            for (Extension ex : exts.getAllExtensions()) {
                if (ex.isCritical()) {
                    extSet.add(((sun.security.x509.Extension) ex).getExtensionId().toString());
                }
            }
            return extSet;
        } catch (Exception e) {
        ...
        try {
            ...
            if (extensions == null) {
                ...
                for (Extension ex : extensions.getAllExtensions()) {
                    if (((sun.security.x509.Extension) ex).getExtensionId().equals(oid)) {
    ...
    public byte[] getExtensionValue(String oid) {
        try {
            ...
            if (extAlias == null) { // may be unknown
                ...
                for (Extension ex : exts.getAllExtensions()) {
                    ObjectIdentifier inCertOID = ((sun.security.x509.Extension) ex).getExtensionId();
            ...
            byte[] extData = ((sun.security.x509.Extension) certExt).getExtensionValue();
            if (extData == null) {
                return null;
            }
            DerOutputStream out = new DerOutputStream();
    ...
    public static byte[] getEncodedInternal(Certificate cert)
        ...
        if (cert instanceof X509) {
            return ((X509)cert).getEncodedInternal();
        } else {
